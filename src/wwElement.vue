  ...
  <div class="alc-wrapper">
    ...
    >
      ...
      <div class="alc-row">
        ...
        <span class="alc-value">{{ item.displayUser }}</span>
      </div>
      <div class="alc-row">
        <span class="alc-label">Action</span>
        <span class="alc-value">{{ item.displayAction }}</span>
      </div>
      <div class="alc-row">
        <span class="alc-label">Target</span>
        <span class="alc-value">{{ item.displayTarget }}</span>
      </div>
    </div>
  ...
  setup(props, { emit }) {
    ...
    const allItems = computed(() => {
      ...
      return items.map((item) => {
        ...
        const displayAction = resolveMappingFormula(props.content && props.content.dataNameFormula, item) || item.action || '';
        const displayUser = resolveMappingFormula(props.content && props.content.dataUserFormula, item) || item.user_id || '';
        const displayTarget = resolveMappingFormula(props.content && props.content.dataTargetFormula, item) || item.target_type || '';
    ...
    // Unique actions for dropdown — use raw action key for filtering
    const uniqueActions = computed(() => {
      const seen = {};
      const result = [];
      const items = allItems.value;
      for (let i = 0; i < items.length; i++) {
        const a = items[i].displayAction || '';
        if (a && !seen[a]) {
          seen[a] = true;
          result.push(a);
        }
    ...
    const filteredItems = computed(() => {
      ...
      return items.filter((item) => {
        const matchSearch = !search ||
          ...
          (item.displayUser + '').toLowerCase().indexOf(search) !== -1 ||
          (item.displayAction || '').toLowerCase().indexOf(search) !== -1 ||
          (item.displayTarget || '').toLowerCase().indexOf(search) !== -1;

        const matchAction = !actionVal || item.displayAction === actionVal;
    ...
    const goToPage = (page) => {
      ...
      emit('trigger-event', {
        name: 'page-change',
        event: { page: clamped, pageSize: resolvedPageSize.value, offset },
      });
    };
